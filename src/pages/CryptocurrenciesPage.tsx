...
function Sparkline({ points }: { points?: number[] }) {
  if (!points || points.length === 0)
    return <span className="text-xs opacity-60">нет данных</span>
  const width = 110,
    height = 34,
    padding = 2
...
export default function CryptocurrenciesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((c) => {
      ...
      if (moveFilter === "gainers")
        return (c.price_change_percentage_24h ?? 0) >= 0
      if (moveFilter === "losers")
        return (c.price_change_percentage_24h ?? 0) < 0
      return true
    })
  ...
  const orderLabel = useMemo(
    () => ORDER_OPTIONS.find((o) => o.value === order)?.label ?? order,
    [order]
  )
  ...
  const columns: ColumnsType<Coin> = [
      ...
      render: (_, r) => (
        ...
        >
          ...
          <div className="flex flex-col leading-tight">
            <span className="text-[13px] font-semibold text-slate-900 group-hover:underline dark:text-slate-100">
              {r.name}
            </span>
...

    {
      ...
      width: 140,
      responsive: ["lg"],
      render: (_, r) => <Sparkline points={r.sparkline_in_7d?.price} />,
    },
  ]
        ...
        <div className="rounded-2xl border border-black/10 bg-white p-5 dark:border-white/10 dark:bg-[#0e141f]">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <Segmented
              ...
              value={moveFilter}
              onChange={(v) => setMoveFilter(v as any)}
            />
          </div>
        </div>
      </motion.div>
