...
export default function CryptocurrenciesPage() {
  ...
  const columns: ColumnsType<Coin> = [
    ...
    },
    {
      ...
      render: (_: any, r: Coin) => (
        <div
          onClick={() => nav(`/coin/${r.id}`)}
          className="group flex cursor-pointer items-center gap-3"
        >
          <img src={r.image} alt={r.name} className="h-7 w-7 rounded-full ring-1 ring-white/10" />
          <div className="flex flex-col leading-tight">
            <span className="text-[13px] font-semibold text-white group-hover:underline">{r.name}</span>
            <span className="text-xs uppercase tracking-wide text-slate-400">{r.symbol}</span>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (v: number) => <span className="font-medium">{nfUSD.format(v)}</span>
    },
    {
      title: () => (
        <Space size={6}>
    ...
    },
    {
      ...
      render: (v: number) => nfUSD.format(v)
    },
    {
      title: 'Volume 24h',
        ...
        >
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <Tag className="rounded-full border-white/20 bg-white/10 px-3 py-2 text-white">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              />
              <Segmented
                options={[{ label: 'Top 20', value: 20 }, { label: 'Top 50', value: 50 }, { label: 'Top 100', value: 100 }]}
                value={limit}
